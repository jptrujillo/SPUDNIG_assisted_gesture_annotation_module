...
updated 23-04-2021 for SPUDNIG2.0.2
@jptrujillo
...
def rest(i, data, keypoint, fps):
    ...
    for j in range(start, end):
        if abs(data.loc[j][keypoint] - x) < move_threshold and abs(data.loc[j][keypoint + 1] - y) < move_threshold:
            certainty += 1
...
def get_gestures_keypoint(data, keypoint_index, track_threshold,fps):
    '''Recognizes gestures that occur in data based on keypoint 'keypoint' and stores outcomes in gestures.'''
    local_gestures = [0]
    rest_x = 0
    rest_y = 0
    keypoint_index *= 3
    # for MediaPipe, 0.01 is 1% pixels, as the values are scaled (0,1) to the frame size of the video
    ...
    while i < data.shape[0] - 1:
        i = i + 1
        gesture = 0
        # "current" is a Series object, containing x,y,confidence values for each keypoint
        # the current keypoint being processed is specified by the keypoint_index input parameter
        current = data.loc[i]
        prev = data.loc[i - 1]
        returned = False
        backToRest = -1
        ...
        if current[keypoint_index + 2] < track_threshold:
            local_gestures.append(gesture)

        else:
            # update rest position
            if rest(i, data, keypoint_index, fps):
                rest_x = current[keypoint_index]
                rest_y = current[keypoint_index + 1]
                gesture = 0

            # if hand coordinates are different from previous frame
            elif abs(current[keypoint_index] - rest_x) > move_threshold or abs(current[keypoint_index + 1] - rest_y) > move_threshold:
                # check if it's actual movement or just a few frames
                certainty = 0
                for x in range(i + 1, min(i + 6, data.shape[0])):
                    if abs(data.loc[x][keypoint_index] - rest_x) > move_threshold or abs(data.loc[x][keypoint_index + 1] - rest_y) > move_threshold:
                        certainty += 1
                ...
                # if there is indeed movement
                    ...
                    if not returned:
                        for t in range(i + 1, min(i + 300, data.shape[0])):
                            if rest(t, data, keypoint_index, fps):
                                gesture = 1
                                returned = True
                                backToRest = t
...
def elan_writer(list_of_gestures, fps):
    ...
    while idx < len(list_of_gestures) - 1:
        ...
        if val == 1:
            begin = frameToTime(idx, fps)  
            end = ''
            for idx2, val2 in enumerate(list_of_gestures[idx + 1:]):
                if val2 == 0:
                    end = frameToTime(idx + idx2, fps)
                    break
            elan_list.append(['Movements', begin, end,  'movement'])
            idx = idx + idx2 + 1
        else:
            idx += 1
    elan = pd.DataFrame(elan_list, columns = ['Tier', 'Begin', 'End', 'Annotation'])
    return elan


def main(root, threshold, keypoints_left, keypoints_right, keypoints_body, fps, min_cutoff, gap_cutoff):
    try:
        pose = pd.read_csv(posixpath.join(root, 'body_sample.csv'), header=None)
        indices_body = range(int(len(pose.columns)/3))
        
    except:
        pose= []
        indices_body = []
        print( "No body movement detected")
    try:
        left_hand = pd.read_csv(posixpath.join(root, 'hand_left_sample.csv'), header=None)
        indices_left = range(int(len(left_hand.columns)/3))
    except:
        left_hand = []
        indices_left = []
        print("No left hand movement")
    try:
        right_hand = pd.read_csv(posixpath.join(root, 'hand_right_sample.csv'), header=None)
        indices_right = range(int(len(right_hand.columns)/3))
    except:
        right_hand = []
        indices_right = []
        print("No right hand movement")
    ...
    movement = []
    if indices_left:
        movement = get_gestures_keypoint(left_hand, indices_left[0], threshold,fps)
        for index in indices_left[1:]:
            movement = merge_gestures(movement, get_gestures_keypoint(left_hand, index, threshold,fps))

    if indices_right:
        if not movement:
            movement = get_gestures_keypoint(right_hand, indices_right[0], threshold,fps)
        else:
            movement = merge_gestures(movement, get_gestures_keypoint(right_hand, indices_right[0], threshold,fps))
        for index in indices_right[1:]:
            movement = merge_gestures(movement, get_gestures_keypoint(right_hand, index, threshold,fps))

    if indices_body:
        if not movement:
            movement = get_gestures_keypoint(pose, indices_body[0], threshold,fps)
        else:
            movement = merge_gestures(movement, get_gestures_keypoint(pose, indices_body[0], threshold,fps))
        for index in indices_body[1:]:
            movement = merge_gestures(movement, get_gestures_keypoint(pose, index, threshold,fps))
